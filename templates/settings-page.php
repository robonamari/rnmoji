...
function rnmoji_settings_page(): void
{
    ...
    <div class="wrap">
        ...
        <form method="post" enctype="multipart/form-data" novalidate>
            ...
            <p>
                <?= esc_html__(
                    "You can upload up to 2000 custom emojis.",
                    "rnmoji"
                ) ?><br>
                <strong><?= esc_html__(
                    "Upload Requirements:",
                ...
                - <?= esc_html__(
                    "Recommended dimensions: 62 x 62 pixels",
                    "rnmoji"
                ) ?><br>
                - <?= esc_html__(
        ...
        <table class="wp-list-table widefat fixed striped" role="grid" aria-describedby="emoji-table-description">
            <caption id="emoji-table-description" class="screen-reader-text">
                <?= esc_html__(
                    "List of uploaded emojis with actions",
                    "rnmoji"
                ) ?>
            </caption>
            <thead>
            ...
            <tbody>
                ...
                foreach ($emoji_files as $file):
                    ...
                    <tr>
                        <td><?= $index++ ?></td>
                        <td><img src="<?= esc_url($url) ?>" alt="<?= esc_attr(
    $name
) ?>" width="25" height="25" loading="lazy"></td>
                        <td>
                            <form method="post" style="display:flex; gap:0.5rem;" novalidate>
                                <input type="hidden" name="old_emoji" value="<?= esc_attr(
                                    $file
                                ) ?>">
                                <input
                                    type="text"
                                    ...
                                    title="<?= esc_attr__(
                                        "At least 2 chars, alphanumeric and underscores only",
                                        "rnmoji"
                                    ) ?>"
...
{
    if (
        !file_exists(RNMOJI_UPLOAD_DIR) &&
        !mkdir(RNMOJI_UPLOAD_DIR, 0777, true) &&
        !is_dir(RNMOJI_UPLOAD_DIR)
    ) {
        echo '<div class="error"><p>' .
            esc_html__("Unable to create upload directory.", "rnmoji") .
            "</p></div>";
        return;
    }
    ...
    if (count($emoji_files) >= 2000) {
        echo '<div class="error"><p>' .
            esc_html__(
                "You have reached the maximum limit of 2000 emojis. Upload not allowed.",
                "rnmoji"
            ) .
            "</p></div>";
        return;
    ...
    if (!preg_match('/^[A-Za-z0-9_]{2,}$/', $emoji_name)) {
        echo '<div class="error"><p>' .
            esc_html__(
                "Emoji name must be at least 2 characters and only contain letters, numbers, and underscores.",
                "rnmoji"
            ) .
            "</p></div>";
        return;
    ...
    if (file_exists($target_path)) {
        echo '<div class="error"><p>' .
            esc_html__(
                "File name is already taken. Emoji not uploaded.",
                "rnmoji"
            ) .
            "</p></div>";
        return;
...
function upload_backup(): void
{
    if (!isset($_FILES["backup_file"])) {
        echo '<div class="error"><p>' .
            esc_html__("No backup file uploaded.", "rnmoji") .
            "</p></div>";
        return;
    }
    ...
    if (
        ...
        echo '<div class="error"><p>' .
            esc_html__("Failed to create emoji directory.", "rnmoji") .
            "</p></div>";
        $zip->close();
        return;
    }
...
function rename_emoji(): void
{
    ...
    if (!preg_match('/^[A-Za-z0-9_]{2,}$/', $new)) {
        echo '<div class="error"><p>' .
            esc_html__(
                "Emoji name must be at least 2 characters and contain only letters, numbers, and underscores.",
                "rnmoji"
            ) .
            "</p></div>";
        return;
